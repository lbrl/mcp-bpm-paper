...
\begin{abstract}
  ...
  beam at J-PARC and additionally with a ultraviolet (UV) light source.  It has
...
rate, and the intensity of a few $10^{6~}\mu^{+}$\SI{}{/\s}~\cite{D-line, D-line1}.  The beam intensity was adjusted by
...
The scale of a pixel in the
picture is determined as \SI{0.08}{\mm \per \pixel} correlating
...
fluctuation in RMS value (\SI{8.7}{ADC{~}counts/\pixel}) from each offset ADC counts in each pixel. The properties of single
...
($t_{delay} = 2~$or$~\SI{3}{\micro\s}$), the minimum width distribution is
